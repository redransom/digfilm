...
if($('#jstwitter').length) {$.include('/js/twitterFetcher.js');}
if($('.star').length) {$.include('/js/jquery.raty.min.js');}
if($('#scroll-pane').length) {$.include('/js/jquery.mousewheel.min.js');}
if($('#scroll-pane').length) {$.include('/js/jquery.jscrollpane.min.js');}
...
/* Rating --> Begin */
	if($('.star').length) {$('.star').raty({half:  true, score: 3.3});}
/* Rating --> End */
...
/* Prepare loading fancybox --> Begin */
	...
	if($('#various, #login, #create').length) {
		jQuery('#various, #login, #create').fancybox({
